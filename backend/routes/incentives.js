...
router.post('/goals', authMiddleware, requirePermission('hr.manage'), async (req, res) => {
  try {
    const g = req.body;
    if (!g.name || !['unit_threshold', 'promoter_target'].includes(g.type)) {
      return res.status(400).json({ error: 'name e type (unit_threshold|promoter_target) são obrigatórios' });
    }
    ...
    const { rows } = await pool.query(
      `INSERT INTO staff_incentive_goals
         (name, type, product_id, every_n_units, target_value, base_amount, full_amount, employee_ids, notes, created_by)
...
async function computeBonusReport(from, to) {
  ...
  const { rows: orders } = await pool.query(
    `SELECT created_by, items FROM orders
  ...
  for (const o of orders) {
    if (!perEmployee.has(o.created_by)) {
      perEmployee.set(o.created_by, { total: { units: 0, revenue: 0 }, byProduct: new Map() });
    }
